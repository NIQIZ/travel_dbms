...
**Issue:** "Index not used in query"  
**Solution:** Check `EXPLAIN QUERY PLAN` output. Ensure WHERE clause matches index column order.

**Issue:** "MongoDB COLLSCAN instead of IXSCAN"  
**Solution:** Create compound index matching `$match` and `$sort` stages.

**Issue:** "Slow aggregation queries"  
**Solution:** Move `$match` and `$sort` stages to beginning of pipeline.

**Issue:** "Optimistic locking always fails"  
**Solution:** Ensure frontend sends current `version` field in update payload.

**Issue:** "High memory usage during queries"  
**Solution:** Implement pagination, use projection to limit fields, enable query result streaming.
...
- MongoDB Profiler: `db.setProfilingLevel(2)`
- Chrome DevTools: Network tab for API latency
