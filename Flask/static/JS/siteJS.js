...
function logPerformance(logs) {
  ...
  logs.forEach((log) => {
    ...
    const isIndexUsed =
      planStr.includes("USING INDEX") ||
      planStr.includes("COVERING INDEX") ||
      planStr.includes("IXSCAN");
    const isScan =
      planStr.includes("SCAN TABLE") || planStr.includes("COLLSCAN");
    ...
    let planHtml = log.plan
      ? log.plan.map((p) => `<div>${p}</div>`).join("")
      : "";
...
async function loadDashboardData() {
  ...
  try {
    ...
    const [flightOps, routePerf, passengerDemand, revenue, resources] =
      responses;
    ...
    if (flightOps) {
      ...
      safeRender(
        createPunctualChart,
        flightOps.least_punctual_routes,
        "Punctuality"
      );
    }
    ...
    if (passengerDemand) {
      safeRender(
        ...
        passengerDemand.top_occupancy_routes,
        "Occupancy"
      );
      safeRender(
        createMarketShareChart,
        passengerDemand.busiest_routes_market_share,
        "Market Share"
      );
      safeRender(
        createLeastBusyChart,
        passengerDemand.least_busy_routes,
        "Least Busy"
      );
    }
    ...
    if (revenue) {
      try {
        createProfitableRoutesChart(
          revenue.top_revenue_routes,
          revenue.least_revenue_routes
        );
      } catch (e) {
        console.error("Failed Profit Chart", e);
      }

      safeRender(
        createRevenueClassChart,
        revenue.revenue_by_class,
        "Rev Class"
      );
      safeRender(
        createRevenueClassRouteChart,
        revenue.revenue_by_class_route,
        "Rev Route"
      );
    }
    ...
    if (resources) {
      safeRender(
        createAircraftRouteChart,
        resources.aircraft_by_route,
        "Matrix Chart"
      );
      safeRender(
        createDestinationsChart,
        resources.top_destinations,
        "Destinations"
      );
      safeRender(
        createUtilizationChart,
        resources.aircraft_utilization,
        "Utilization"
      );
...
function updateKeyMetrics(overview) {
  ...
  document.getElementById("totalFlights").textContent = (
    overview.total_flights || 0
  ).toLocaleString();
  let ontimeRate = 0;
  if (overview.total_flights > 0) {
    ontimeRate = (
      (overview.ontime_flights / overview.total_flights) *
      100
    ).toFixed(1);
  }
  document.getElementById("ontimeRate").textContent = ontimeRate + "%";
  document.getElementById("avgDelay").textContent =
    overview.avg_delay_minutes || 0;
  document.getElementById("cancellations").textContent = (
    overview.cancelled_flights || 0
  ).toLocaleString();
}
  ...
  createOrUpdateChart("punctualChart", {
    ...
    options: {
      ...
      scales: {
        x: {
          beginAtZero: true,
          title: { display: true, text: "Average Delay (mins)" },
        },
      },
    },
  });
...
function createMarketShareChart(data) {
  ...
  const sortedData = [...data].sort(
    (a, b) => b.market_share_percent - a.market_share_percent
  );
  ...
  createOrUpdateChart("marketShareChart", {
    ...
    data: {
      ...
      datasets: [
        ...
        // DATASET 2: THE STICK (Faded Blue Stick)
        {
          type: "bar",
          label: "Stick",
          data: sortedData.map((d) => d.market_share_percent),
          backgroundColor: "rgba(33, 150, 243, 0.5)",
          barPercentage: 0.1, // <--- Makes it a stick
      ...
      plugins: {
        ...
        tooltip: {
          filter: function (tooltipItem) {
            return tooltipItem.datasetIndex === 0; // Only tooltip for the Dot
          },
          callbacks: {
...
function createLeastBusyChart(data) {
  ...
  // Sort Smallest to Largest (Ascending) so the lollipops go "up"
  ...
  createOrUpdateChart("leastBusyChart", {
    ...
    data: {
      labels: data.map((d) => d.route),
      datasets: [
        // DATASET 1: THE CANDY (Orange Dot)
        {
          type: "line",
          label: "Market Share %",
        ...
        // DATASET 2: THE STICK (Faded Orange Stick)
        {
          type: "bar",
          label: "Stick",
          data: data.map((d) => d.market_share_percent),
...
function createRevenueClassRouteChart(data) {
    ...
    return {
      ...
      data: sortedRoutes.map((route) => {
        const record = data.find(
          (d) => d.route === route && d.fare_conditions === fareClass
        );
  ...
  createOrUpdateChart("revenueClassRouteChart", {
    type: "bar",
    data: { labels: sortedRoutes, datasets: datasets },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      scales: {
...
function renderFilteredAircraftChart() {
  ...
  if (selectedRoute === "ALL") {
    ...
    routesToShow = [...new Set(cachedAircraftData.map((d) => d.route))].slice(
      0,
      15
    );
    displayData = cachedAircraftData.filter((d) =>
      routesToShow.includes(d.route)
    );
  } else {
  ...
  const colors = {
    SU9: "rgba(255, 99, 132, 0.7)",
    CN1: "rgba(255, 206, 86, 0.7)",
    321: "rgba(75, 192, 192, 0.7)",
    773: "rgba(54, 162, 235, 0.7)",
    763: "rgba(153, 102, 255, 0.7)",
    CR2: "rgba(255, 159, 64, 0.7)",
    319: "rgba(201, 203, 207, 0.7)",
    733: "rgba(100, 221, 23, 0.7)",
  };
  const datasets = allAircraft.map((ac) => {
    return {
      ...
      data: routesToShow.map((route) => {
        const record = displayData.find(
          (d) => d.route === route && d.aircraft_code === ac
        );
  ...
  createOrUpdateChart("aircraftRouteChart", {
    type: "bar",
    data: { labels: routesToShow, datasets: datasets },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      scales: {
        x: {
          ...
          title: {
            display: true,
            text: selectedRoute === "ALL" ? "Top Routes" : "Selected Route",
          },
        },
        y: {
          stacked: true,
...
function createUtilizationChart(data) {
  ...
  // 1. Sort High to Low so the biggest slice is first
  data.sort((a, b) => b.total_mileage - a.total_mileage);

  // 2. Keep only Top 5 planes
  const top5Data = data.slice(0, 5);

  // 3. Define our 5 colors
  ...
  createOrUpdateChart("utilizationChart", {
    type: "polarArea",
    data: {
      // Label format: "Code (Model) : 12k mi"
      labels: top5Data.map(
        (d) =>
          `${d.aircraft_code} (${d.aircraft_model}) : ${(
            d.total_mileage / 1000
          ).toFixed(0)}k mi`
      ),
      datasets: [
        {
...
function createSU9RoutesTable(data) {
  ...
  data.forEach((row) => {
    ...
    tr.innerHTML = `<td>${row.flight_id}</td><td>${row.route}</td><td>${
      row.scheduled_departure_time
    }</td><td><span class="badge bg-${getStatusColor(row.status)}">${
      row.status
    }</span></td>`;
